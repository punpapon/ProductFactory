...
import service.WorkflowProductTask;
...
/**
 * Servlet implementation class ProductCL_Data_default_Servlet
 */
@WebServlet("/ProductCL_Data_default_Servlet")
public class ProductCL_Data_default_Servlet extends HttpServlet {
     ...
     */
    public ProductCL_Data_default_Servlet() {
        super();
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (method.equals("select_insurance_type")) {
			...
			}else if (method.equals("main_contract_level3") && !type_lv1.equals("0") && !type_lv2.equals("0")) {
				//level 3
				...
				if (main_contracttype.equals("1")) {
					...
					///Step 3//////////////////////////////////////////////////////////////////	
						String[] appPage  		= new String[7]; 
							...
							/////////////////////////////////////////////////////////////////////////////////
							new WorkflowProductTask().insertWorkflowProductTask(passInsurance, userId);
				}
				...
				MainInsureDetailCreateProduct mdcs = new MainInsureDetailCreateProduct();
				mdcs.createDetail(1);
				ArrayList<LookupCatAccessData> datareturn_LookupCat = mdcs.getListLookupCatAccessData();
				...
				if (menu_CL_1.equals("true")) {
					...
					ArrayList<SalesChannelAccessData> datareturn_Sale_Channel = mdcs.getListSaleChannelAccessData() ;
					...
					ArrayList<RateCatAccessData> datareturn = mdcs.getListNar();
					...
					ArrayList<RateCatAccessData> datareturn2 = mdcs.getListPMRate(); 
				...
				if (menu_CL_2.equals("true")) {
					...
					/*for (int i = 0 ; i < datareturn_LookupCat.size(); i++) {
						//Unit
						if (datareturn_LookupCat.get(i).getLookupCat().getEnum_name().equals("lu_cov_unit_type")) {
							for (int ii = 0; ii < datareturn_LookupCat.get(i).getListLookupValue().size(); ii++) {
								JSONObject json_unit = new JSONObject();
								json_unit.put("pass",  datareturn_LookupCat.get(i).getListLookupValue().get(ii).getEnum_value());
								json_unit.put("text",  datareturn_LookupCat.get(i).getListLookupValue().get(ii).getName_th());
								unit_jsonArray.put(json_unit);
							}
						}
						//OF
						if (datareturn_LookupCat.get(i).getLookupCat().getEnum_name().equals("lu_cov_factor")) {
							for (int ii = 0; ii < datareturn_LookupCat.get(i).getListLookupValue().size(); ii++) {
								JSONObject json_or = new JSONObject();
								json_or.put("pass",  datareturn_LookupCat.get(i).getListLookupValue().get(ii).getEnum_value());
								json_or.put("text",  datareturn_LookupCat.get(i).getListLookupValue().get(ii).getName_th());
								of_jsonArray.put(json_or);
							}
						}
					}*/
...
/*					//สูตรการคำนวณเบี้ย
					JSONArray	cal_Formula_Insurance_jsonArray	= new JSONArray();
					String [] cal_Formula_Insurance_pass = {"1"};
					String [] cal_Formula_Insurance_text = {"สูตร 1"};
				
					for (int i = 0; i < cal_Formula_Insurance_pass.length; i++) {
						JSONObject json_cal_Formula_Insurance = new JSONObject();
						json_cal_Formula_Insurance.put("pass", cal_Formula_Insurance_pass[i]);
						json_cal_Formula_Insurance.put("text", cal_Formula_Insurance_text[i]);
						cal_Formula_Insurance_jsonArray.put(json_cal_Formula_Insurance);
					}
*/					
				...
				if (menu_CL_4.equals("true")) {
					...
					/*System.out.println("------/"+sales_benefits_jsonArray.length());
					for (int j = 0; j < sales_benefits_jsonArray.length(); j++) {
						System.out.println("====/"+sales_benefits_jsonArray.get(j).toString());
					}*/
				...
				if (menu_CL_5.equals("true")) {
					...
					for (int i = 0 ; i < datareturn_LookupCat.size(); i++) {
						...
						/*if (datareturn_LookupCat.get(i).getLookupCat().getEnum_name().equals("lu_cov_unit_type")) {
							for (int ii = 0; ii < datareturn_LookupCat.get(i).getListLookupValue().size(); ii++) {
								JSONObject json_fix_cov_unit = new JSONObject();
								json_fix_cov_unit.put("pass",  datareturn_LookupCat.get(i).getListLookupValue().get(ii).getEnum_value());
								json_fix_cov_unit.put("text",  datareturn_LookupCat.get(i).getListLookupValue().get(ii).getName_th());
								fix_cov_unit_jsonArray.put(json_fix_cov_unit);
							}
						}*/
						//OF	// pun fix 
						/*if (datareturn_LookupCat.get(i).getLookupCat().getEnum_name().equals("lu_cov_factor")) {
							for (int ii = 0; ii < datareturn_LookupCat.get(i).getListLookupValue().size(); ii++) {
								JSONObject json_fix_cov_factor = new JSONObject();
								json_fix_cov_factor.put("pass",  datareturn_LookupCat.get(i).getListLookupValue().get(ii).getEnum_value());
								json_fix_cov_factor.put("text",  datareturn_LookupCat.get(i).getListLookupValue().get(ii).getName_th());
								fix_cov_factor_jsonArray.put(json_fix_cov_factor);
							}
						}*/
						//unw_hist_rule
					...
					LookupProductData lpd1 = new LookupProductData();
					ArrayList<AppendixRelationAccessData>  arr_Appendix =	lpd1.searchAppendixRelation();
...
////////////////////////////					
					...
					ArrayList<LookupCatAccessData> catValueAcessData_arr = mdcs.getListLookupCatAccessData(); 
				...
				ArrayList<ProductRiderAccessData> listCoverageRider = pbf.searchRider();
		...
		}//Benz get lookup Clone 28-10-2019
		else if (method.equals("create_ProductCL_Clone")) {     
